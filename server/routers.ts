...
export const appRouter = router({
  ...
  maintenances: router({
    ...
    getByVessel: adminProcedure
      ...
      .query(async ({ input }) => {
        return await db.getMaintenancesByVessel(input.vesselId);
      }),

    getByDateRange: adminProcedure
      .input(z.object({ startDate: z.number(), endDate: z.number() }))
      .query(async ({ input }) => {
        return await db.getMaintenancesByDateRange(input.startDate, input.endDate);
      }),
    ...
    create: adminProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        if (input.startDate >= input.endDate) {
          throw new TRPCError({ code: 'BAD_REQUEST', message: 'Data de início deve ser anterior à data de término' });
        }
