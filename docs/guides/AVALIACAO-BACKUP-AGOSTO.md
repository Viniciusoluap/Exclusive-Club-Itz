...
| ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `users`                  | O dump usa `openId`, `loginMethod`, `createdAt`, `updatedAt` e `lastSignedIn`; o baseline físico da `main` também usa esses nomes camelCase | O preparador preserva esses identificadores na cópia `restore.sql`; o ZIP original permanece intacto                                                                        |
...
A cópia preparada faz duas coisas controladas: valida as tabelas obrigatórias e os marcadores de conclusão; e remove a view legada `financial_charges`. Os identificadores camelCase de `users` são preservados porque coincidem com `drizzle/0000_initial_baseline.sql` e com o schema físico da aplicação.
...
A sequência esperada é adoção do baseline, seguida da aplicação das migrações incrementais. Essa sequência foi validada em MariaDB local: a cópia corrigida foi importada com 30 tabelas, o baseline foi adotado, as migrações 0001 e 0002 foram reconhecidas/aplicadas conforme o estado existente, e 0003 a 0008 concluíram; as tabelas Open Finance foram criadas. A collation 0003 foi adaptada apenas na cópia local para `utf8mb4_unicode_ci`, porque MariaDB 10.11 não implementa `utf8mb4_0900_ai_ci`; a migração oficial da produção não foi alterada. Qualquer erro de chave única, coluna incompatível ou dado conflitante deve interromper a promoção, e não ser ignorado.
