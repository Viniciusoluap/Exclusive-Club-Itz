...
22. **Ação admin de teste de conexão Pluggy — auditoria confirma que já existia, faltava só teste (30/08/2026):** pedido para adicionar uma ação administrativa visível, protegida por `adminProcedure`, validando credenciais sandbox só com `POST /auth` + `GET /connectors?countries=BR`, sem Connect Token, sem webhook, sem consentimento, sem contas/transações, sem gravar no banco, com resposta sanitizada. Auditoria linha a linha confirmou que `systemSettings.testPluggyConnection` (`server/routers/systemSettingsRouter.ts`, chamando `openFinance.testPluggyConnection()`) já implementava exatamente isso, com botão visível "Testar API Pluggy" no painel admin (`client/src/pages/SystemSettings.tsx`) — só faltava cobertura de teste (PR #136 em `Exclusive-Club-Itz`, replicada na PR #14 em `Exclusive-Club-Itz-Manus`).

    **Revisão automatizada (Codex) na PR #136 encontrou 2 problemas reais**, ambos confirmados antes de corrigir:
    1. **(Segurança, código de produção)** `parseResponse()` repassava o texto de erro da Pluggy verbatim para o admin — nada garantia que a API nunca ecoasse de volta um `clientId`/`clientSecret` submetido dentro de um erro de autenticação, o que vazaria a credencial. Corrigido com `redactPluggyCredentials()` em `server/openFinance.ts`: resolve a config uma vez em `testPluggyConnection()` e remove qualquer ocorrência literal do client id/secret configurados da mensagem antes de devolvê-la.
    2. **(Teste)** `resolvePluggyConfig()` prioriza `ENV` sobre o `getSetting` mockado (`ENV.pluggyClientId || fromDb`) — sem isolar `ENV`, o teste dependia de `PLUGGY_CLIENT_ID`/`SECRET` não estarem definidas na máquina, e quebraria (2 testes) em qualquer ambiente que os tivesse. Corrigido mockando `./_core/env` por inteiro; confirmado manualmente rodando a suíte com essas variáveis setadas no ambiente — 4/4 continua verde.

    **Incidente operacional no merge (`Exclusive-Club-Itz`):** a PR #136 foi mergeada (squash) pelo responsável **antes** do push do commit de correção (`3c496a8`) chegar ao GitHub — a `main` ficou brevemente sem a redação de credenciais. Corrigido com uma PR de acompanhamento (PR #137, só com o commit que faltou), que por sua vez esbarrou num conflito clássico de "squash-merge seguido de continuação" (o arquivo de teste aparecia como "adicionado nos dois lados" — a `main` via o squash da PR #136, a branch via sua própria história linear) — resolvido com um merge da `main` na branch, mantendo a versão corrigida. Além disso, o CI de `Exclusive-Club-Itz` não disparou automaticamente em dois momentos: (a) o push do commit de correção para uma branch sem PR aberta (PR #136 já estava fechada), e (b) a abertura da PR #137 reaproveitando essa mesma branch — em ambos os casos foi necessário disparo manual via `workflow_dispatch` (`ci.yml` já previa esse cenário, documentado em comentário no próprio arquivo). Nenhum desses dois problemas ocorreu no espelho (`Exclusive-Club-Itz-Manus`), onde a réplica final foi feita direto do conteúdo já corrigido, numa branch nova.

    Dado o Codex ter atingido o limite de uso de revisão automatizada durante essa sequência, a revisão final da réplica (PR #14) foi feita diretamente, linha a linha, sem achados novos.

    Validado localmente nos dois repositórios (`tsc --noEmit` limpo, testes passando, `pnpm run build` sem erro) e no CI oficial — `Exclusive-Club-Itz`: PR #136 (squash `b6acd23c0ded38c433fdb05b18db4d5950ef7586`) + PR #137 (squash `cb2e9922865c8b0e0e19ec73d5fdc95588b2e649`); `Exclusive-Club-Itz-Manus`: PR #14 (squash `e31c9f223d2d65254c075b884df0b9fc7352d663`).

    **Próximo passo exato:** com a ação de teste Pluggy publicada e validada nos dois repositórios, o Manus pode preencher `PLUGGY_CLIENT_ID`/`PLUGGY_CLIENT_SECRET` via My Browser no navegador do responsável e usar o botão "Testar API Pluggy" para validar as credenciais sandbox antes de qualquer consentimento real (Connect Widget).

23. **Mudança de estratégia para a restauração do backup de agosto — mesclagem seletiva dentro da própria aba de Backups, sem trocar `DATABASE_URL` (31/08/2026):** o responsável observou que o backup de agosto ainda não estava restaurado (candidato de staging do item 14 nunca foi promovido — GATE MANUS #2 seguia bloqueado pela rota A: `DATABASE_URL` é injetada pela própria plataforma do Manus e não aparece na lista de Segredos editável; o Manus tentou cadastrar um segredo com esse nome literal e confirmou por telemetria que não sobrepõe a conexão gerenciada — sem caminho de self-service, só suporte externo). O responsável rejeitou esperar por suporte e propôs a alternativa que foi implementada: em vez de um script externo que precisaria da string de conexão real, a restauração vira uma ação dentro do próprio app publicado — que já roda com a conexão real via `DATABASE_URL` injetado pela plataforma, sem que o código nunca precise ler ou expor esse valor.

    **Desenho aprovado pelo responsável** (duas decisões confirmadas antes de implementar): (a) em conflito de chave, a produção atual sempre vence — a linha do backup é descartada, nunca sobrescreve; (b) tabelas sem uma chave natural/`UNIQUE` de verdade no schema (`expense_records`, `bookings`, `client_quotas`, `due_date_change_requests`, `fuel_records`, `fuel_purchases`, `inspections`, `inspection_charges`, `maintenances`, `reviews`, `vessels`, entre outras) **nunca são inseridas automaticamente** — o dry-run só mostra a contagem; o "Aplicar" nunca grava nelas.

    **Implementado:** novo card "Recuperar registros de um backup antigo" em `/admin/backups` (`client/src/pages/admin/Backups.tsx`). Admin sobe o `.zip`/`.sql` ali mesmo; duas rotas Express dedicadas fora do tRPC por causa do tamanho do upload (`server/backupRestoreMergeRoute.ts`, multipart + gate de admin, mesmo padrão de `/api/upload-*`) chamam a lógica de mesclagem (`server/backupRestoreMerge.ts`, reaproveitando as primitivas de parsing de `scripts/prepare_backup_restore.mjs`): **Analisar** (dry-run, só leitura, relatório por tabela) e **Aplicar** (insere só o que falta, nunca `UPDATE`/`DELETE`). Tabelas sensíveis (`system_settings`, `webhook_logs`) e de controle (`backup_history`, `__drizzle_migrations`) nunca são processadas. Testado contra banco real no CI (`server/backupRestoreMerge.test.ts`, mesmo padrão `describe.skipIf(!db)` de `backupCleanup.test.ts`): linha nova inserida, linha existente preservada, idempotência, cobrança sem `asaas_charge_id` nunca entra sozinha, tabela sem chave nunca recebe insert automático.

    **Autorização do responsável (31/08/2026):** "Não me importo de perder dados atuais... o mais importante é recuperar os dados e voltar a operação, sem gastar mais créditos no Manus." Como a mesclagem é puramente aditiva (nunca apaga/sobrescreve), essa tolerância a perda não foi necessária para a via já implementada — mas autoriza, se o dry-run mostrar lacunas relevantes nas tabelas "só contagem" acima, estender a ferramenta para tratá-las também, aceitando o risco de duplicata (não de perda) nessas tabelas específicas.

    Validado (`tsc --noEmit`, `pnpm run build`, suíte de testes) e mergeado nos dois repositórios: `Exclusive-Club-Itz` PR #138 (squash `c6ff02203922af67c7a622be7e193d31e8e7cd25`), `Exclusive-Club-Itz-Manus` PR #15 (squash `3ed36936b03e32cb54d29194a080f705aa86713c`).

    **GATE MANUS #2 original (troca de `DATABASE_URL`) fica superado por esta via** — não é mais necessário trocar a conexão principal para recuperar o backup; a leitura/escrita acontece direto contra a produção real, pelo próprio app.

24. **Primeiro uso real em produção (01/09/2026) — correção de confiabilidade + recuperação forçada das tabelas sem chave:** o responsável executou o item 23 em produção. Resultado: `allowed_clients`/`asaas_customers`/`bpo_charges`/índice de anexos entraram corretamente; `users` também. `employees` (1 registro no backup) foi reportado como inserido pela tela, mas não apareceu depois na tela de Funcionários — o relatório da época só confiava na ausência de erro do `INSERT`, sem reler o banco, e escondia linhas "sem novidade" (exatamente o tipo de caso que ficava sem diagnóstico).

    **Corrigido:** `dryRunRestoreMerge`/`applyRestoreMerge` ficaram isolados por tabela (uma falha não impede as demais, erro fica visível) e **auto-verificados** — depois do `INSERT`, relê o banco pelas mesmas chaves e só marca sucesso quando a contagem confirmada bate com a tentada. A tela parou de filtrar linhas sem novidade e ganhou uma coluna "Em produção hoje". `fetch()` do cliente lê a resposta como texto antes de `JSON.parse`, para nunca deixar um erro de proxy/timeout aparecer como texto cru na tela.

    **Nova capacidade — recuperação forçada por id:** o responsável confirmou que precisa recuperar também vistorias/abastecimentos/reservas/despesas/embarcações etc. (as tabelas "só contagem" do item 23), e que isso é pré-requisito para os anexos (fotos/documentos) voltarem a ser recuperáveis — o "Arquivar anexos" já existente busca as URLs de foto/documento justamente dentro dessas tabelas (`fuel_records`, `fuel_record_containers`, `inspections`, `allowed_clients`, ver `server/backupFiles.ts`), então sem os registros de volta não há nada para ele encontrar. Nova função `forceRestoreTablesWithoutNaturalKey` (`server/backupRestoreMerge.ts`) e botão "Recuperar tabelas sem chave mesmo assim" em `/admin/backups`: insere por id (a PK do backup), pulando só ids que já existem hoje — nunca sobrescreve, mas diferente da chave natural não detecta o mesmo evento recadastrado depois com outro id, então pode duplicar por conteúdo. Risco aceito explicitamente pelo responsável ("Não me importo de perder dados atuais... o mais importante é recuperar os dados") para priorizar a recuperação.

    Validado (`tsc --noEmit`, `pnpm run build`, suíte de testes com banco real no CI) e mergeado nos dois repositórios: `Exclusive-Club-Itz` PR #139 (squash `cc74ea6bb8907cc38fb6ad69420566e800c747f5`), `Exclusive-Club-Itz-Manus` PR #16 (squash `93680e926cf656c6f1de653173381430b3ec50c2`).

    **Próximo passo exato — depende de ação do responsável, não de crédito no Manus:** confirmar que o deploy está no ar (mesma ressalva do item 23 — sem visibilidade daqui sobre o deploy automático da plataforma Manus). Depois, em **Admin → Backups**: (1) repetir **Analisar** + **Aplicar** com o relatório novo e confirmar se `employees` realmente falhou ou não; (2) rodar **Recuperar tabelas sem chave mesmo assim**; (3) rodar **Arquivar anexos** de novo para tentar recuperar fotos/documentos (só funciona para URLs ainda acessíveis onde estavam em agosto — arquivo perdido de verdade não volta). Reconciliação com o Asaas (Fase 3) fica para depois disso, por decisão do responsável.
...
> **Autorizado (31/08/2026):** o responsável autorizou explicitamente a promoção — troca reversível de `DATABASE_URL` de produção para o schema candidato restaurado (item 21/22 da seção 0), mantendo a base atual intacta e acessível para rollback imediato. Escopo exato: (a) apontar a `DATABASE_URL` do aplicativo em produção para o schema candidato já validado (contagens batem com o relatório de agosto — item 14; 5 tabelas do Open Finance preservadas — item 21); (b) preservar a conexão/schema anterior sem excluir nada, disponível para reverter a troca a qualquer momento; (c) nenhum `DROP`/`TRUNCATE`/overwrite — só troca de string de conexão. Execução é do Manus (fora do alcance de código/GitHub desta sessão) — ver mensagem pronta na próxima seção.
> **Mecanismo escolhido (31/08/2026):** o Manus apresentou 2 rotas técnicas para executar a troca: **A** — pedir ao suporte da própria plataforma do Manus para trocar a `DATABASE_URL` real pela conexão do schema candidato (troca de string de conexão, sem código novo); **B** — o Claude implementar uma flag protegida no código que faz o app usar `STAGING_DATABASE_URL` como conexão principal quando ativada (rollback = desligar a flag). **Escolhida a rota A.** A rota B foi descartada porque é exatamente o tipo de troca global de conexão de banco por flag em produção que o responsável já havia rejeitado explicitamente ao autorizar a rota de conferência de staging (item 19 — "Não quero uma troca global de banco por flag em produção"), e porque a A não exige nenhuma mudança na lógica de conexão principal do banco, hoje uma única linha sem condicional. Nenhum código foi escrito para a rota B.
> **Superado (31/08/2026, item 23 da seção 0):** a rota A ficou sem caminho de self-service (Manus confirmou por telemetria que `DATABASE_URL` não é sobrescrevível via Segredos, nem por um segredo de mesmo nome) e o responsável não quis esperar suporte externo. A restauração do backup de agosto passou a ser feita por uma via diferente e melhor: mesclagem seletiva dentro da própria aba de Backups, que já roda com a conexão real de produção sem nunca ler/expor `DATABASE_URL` no código. Esta troca de `DATABASE_URL` (GATE MANUS #2 original) não é mais necessária para recuperar os dados do backup — ver item 23.
